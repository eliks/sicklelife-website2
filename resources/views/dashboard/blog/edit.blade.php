            ...
            <div class="col-md-10">
                <div class="panel panel-white">
                    <div class="panel-heading clearfix">
                        <h4 class="panel-title">Edit Blog Article Form</h4>
                    </div>
                    <div class="panel-body">
                        <form method="POST" action="{{ route('dashboard.blog.update', ['blog' => $blogitem]) }}" class="form-horizontal" onsubmit="fetchBodyText();">
                            @method('PUT')
                            @csrf
                            ...
                            <div class="form-group @error('title') has-error  @enderror">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="title" name="title" value="{{ old('title') ?? $blogitem->title }}">
                                    @error('title')
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                        ...
                                        <div class="col-sm-9">
                                            <select style="margin-bottom:15px;" class="form-control" id="headline-image" name="headline-image" onchange="updateHeadlineImage()">
                                                @foreach($images as $image)
                                                    <option value="{{ $image->id }}" {{ old("headline-image") == $image->id ? "selected" : ($blogitem->image_id == $image->id ? "selected" : "") }}>{{ $image->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group @error('brief') has-error  @enderror">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea name="brief" id="brief"  class="form-control" cols="30" rows="10">{{ old('brief') ?? $blogitem->brief }}</textarea>
                                            @error('brief')
                                                <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                                            @enderror
                                        </div>
                                ...
                                <div class="col-sm-10">
                                    <select style="margin-bottom:15px;" class="form-control" name="category">
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ old("category") == $category->id ? "selected" : ($blogitem->category_id == $category->id ? "selected" : "") }}>{{ $category->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('category')
                                        <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                                ...
                                <div class="col-sm-10">
                                    <select multiple class="form-control" name="authors[]">
                                        @foreach($authors as $author)
                                            <option value="{{ $author->id }}" {{  old("authors[]") && in_array($author->id, old("authors[]")) ? "selected" : (in_array($author->id, $blogitem_authors) ? "selected" : "") }}>{{ $author->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('authors')
                                        <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                            ...
                            <div class="form-group @error('listing_date') has-error  @enderror">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control date-picker" id="listing_date" name="listing_date" value="{{ old('listing_date') ?? $blogitem->listing_date }}">
                                    @error('listing_date')
                                        <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group @error('body') has-error  @enderror">
                                ...
                                <div class="col-sm-10">
                                    <div class="summernote">{!! old("body") ?? $blogitem->body !!}</div>
                                    @error('body')
                                        <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                                    @enderror
                                </div>
