            ...
            <div class="col-md-9">
                <div class="panel panel-white">
                    <div class="panel-heading clearfix">
                        <h4 class="panel-title">Edit Album Form</h4>
                    </div>
                    <div class="panel-body">
                        <form method="POST" action="{{ route('dashboard.album.store') }}" class="form-horizontal">
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                        ...
                                        <div class="col-sm-9">
                                            <select style="margin-bottom:15px;" class="form-control" id="headline-image" name="headline-image" onchange="updateHeadlineImage()">
                                                @foreach($images as $image)
                                                    <option value="{{ $image->id }}" {{ old("headline-image") == $image->id ? "selected" : ($album->image_id == $image->id ? "selected" : "") }}>{{ $image->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group @error('description') has-error  @enderror">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea name="description" id="description"  class="form-control" cols="30" rows="10">{{ old('description') ?? $album->description }}</textarea>
                                            @error('description')
                                                <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                                            @enderror
                                        </div>
                            ...
                            <div class="form-group @error('listing_date') has-error  @enderror">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control date-picker" id="listing_date" name="listing_date" value="{{ old('listing_date') ?? $album->listing_date }}">
                                    @error('listing_date')
                                        <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-sm-10">
                                    <select style="margin-bottom:15px;" class="form-control" name="album_category">
                                        @foreach($album_categories as $category)
                                            <option value="{{ $category->id }}"  {{ old('album_category') == $category->id ? "selected" : ($album->category_id == $category->id ? "selected" : "") }}>{{ $category->name }}</option>
                                        @endforeach
                                    </select>
