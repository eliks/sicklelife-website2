            ...
            <div class="col-md-9">
                <div class="panel panel-white">
                    ...
                    <div class="panel-body">
                        <form method="POST" action="{{ route('dashboard.user.update', ['user' => $user]) }}" class="form-horizontal">
                            @method('PUT')
                            @csrf
                                ...
                                <div class="col-sm-10">
                                    <select style="margin-bottom:15px;" class="form-control" name="user_type">
                                        @foreach($user_types as $user_type)
                                            <option value="{{ $user_type->id }}" {{ old('user_type') == $user_type->id ? "selected" : ($user->user_type_id == $user_type->id ? "selected" : "") }}>{{ $user_type->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('user_type')
                                        <p class="help-block" style="margin-bottom:0;">{{ $message }}</p>
